...
def _fetch_services_sync():
    """Sinxron (bloklovchi) so'rov — asyncio.to_thread ichida chaqiriladi."""
    data = {"key": TOPSMM_API_KEY, "action": "services"}
    response = requests.post(TOPSMM_URL, data=data, timeout=15)
    return response.json()


async def get_custom_services(force_refresh: bool = False):
    """Xizmatlar ro'yxatini oladi, 25% ustama qo'shadi, keshlaydi.

    requests.post bloklovchi funksiya, shuning uchun asyncio.to_thread
    orqali alohida thread'da ishga tushiriladi — bot boshqa foydalanuvchilarga
    javob berishda "qotib qolmasligi" uchun.
    """
    now = time.time()
    ...
    try:
        services = await asyncio.to_thread(_fetch_services_sync)
...
async def get_service_by_id(service_id: int):
    for s in await get_custom_services():
        if int(s.get("service")) == service_id:
            return s
    return None


def _create_order_sync(service_id, link, quantity):
    data = {
        "key": TOPSMM_API_KEY,
        "action": "add",
        "service": service_id,
...
async def create_topsmm_order(service_id, link, quantity):
    try:
        return await asyncio.to_thread(_create_order_sync, service_id, link, quantity)
    except Exception as e:
...
async def show_categories(callback: types.CallbackQuery):
    services = await get_custom_services()
    if not services:
...
def _prefilter_services(services: list, query: str, limit: int = 60) -> list:
    """AI'ga yuborishdan oldin ro'yxatni kichraytiradi: nomida yoki
    kategoriyasida so'rovdagi so'zlardan biri uchraydigan xizmatlarni
    ustunlikka qo'yadi. Bu Gemini'ga yuboriladigan matnni qisqartirib,
    javobni sezilarli tezlashtiradi."""
    ...
    matched = [
        s for s in services
        if any(w in (s.get("name", "") + s.get("category", "")).lower() for w in words)
    ]
    if matched:
...
async def process_ai_search(message: types.Message, state: FSMContext):
    ...
    candidates = _prefilter_services(services, user_query)
    ...
    try:
        response = await asyncio.to_thread(
            ...
            contents=prompt,
            config={
                "thinking_level": "low",   # chuqur fikrlash shart emas — javob tezroq keladi
                "max_output_tokens": 50,   # bizga faqat qisqa ID ro'yxati kerak
            },
        )
...
async def confirm_order(callback: types.CallbackQuery, state: FSMContext):
    ...
    result = await create_topsmm_order(data["service_id"], data["link"], data["quantity"])
